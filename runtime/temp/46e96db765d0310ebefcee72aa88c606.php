<?php if (!defined('THINK_PATH')) exit(); /*a:3:{s:88:"D:\phpStudy\PHPTutorial\WWW\thy-cms\public/../application/index\view\category\index.html";i:1558268250;s:81:"D:\phpStudy\PHPTutorial\WWW\thy-cms\application\index\view\public\head_admin.html";i:1554637412;s:77:"D:\phpStudy\PHPTutorial\WWW\thy-cms\application\index\view\public\footer.html";i:1556348597;}*/ ?>
<!DOCTYPE HTML>
...
<script>
    $(function () {
        layui.use('table', function(){
            ...
            var tableOptions={
                elem: '#articleTable'
                ,url: '/public/category/getData' //数据接口
                ,page: true //开启分页
                ,limit:15
                ,limits:[15,30,45,60]
                ,toolbar: '#toolBar'
                ...
                cols:[[
                    ...
                    ,{field: 'name', title: '标题',edit:"text",minwidth:300}
                    // ,{field: 'cat', title: '分类', width:200, sort: true}
                    ,{field: 'img', title: '图片',width:70
                        ,templet: function(d){
                            //调取文章的图片
                            if (d.img1){
            ...
            table.on('edit(article)', function(obj){ //注：edit是固定事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                $.ajax({
                    url: "/public/category/editArticle?id="+obj.data.id,
                    type: 'post',
                    dataType: 'json',
                    data:editData,
            ...
            table.on('tool(article)', function(obj){ //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
                ...
                if(layEvent === 'detail'){ //查看
                    ...
                    layer.confirm('真的删除行么', function(index){
                        ...
                        //提交删除方法
                        $.ajax({
                            url: "/public/category/delArticle",
                            type: 'post',
                            dataType: 'json',
                            data:{
                                id:data.id
                    ...
                    var id=obj.data.id;
                    let text=obj.data.name;
                    let aHref=$.parseHTML("/public/category/edit?id="+id);
            ...
            table.on('toolbar(article)', function(obj){
                ...
                switch(obj.event){
                    case 'add':
                        let text="添加分类";
                        let aHref=$.parseHTML("/public/category/add");
                        let aTarget="admin_body";
                        parent.window.parentAddTab(aHref,text,aTarget);
                        break;
                    case 'delete':
                        ...
                        layer.confirm('真的删除行么', function(index){
                            ...
                            //提交删除方法
                            $.ajax({
                                url: "/public/category/delAllArticle",
                                type: 'post',
                                dataType: 'json',
                                data:{
                                    id:id_str
